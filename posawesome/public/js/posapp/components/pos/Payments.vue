      ...
      <div class="overflow-y-auto px-2 pt-2" style="max-height: 75vh">
        ...
        >
          ...
          <v-col cols="5">
            <v-btn
              color="primary"
              @click="calculate_loyalty_price"
            >
              {{ frappe._('Calculate Loyalty Price') }}
            </v-btn>
          </v-col>
        </v-row>
        ...
        <v-row class="pb-0 mb-2" align="start">
          <v-col cols="12">
            <p
              v-if="sales_persons && sales_persons.length > 0"
    ...
    calculate_loyalty_price() {
      if (this.loyalty_amount > this.available_points_amount) {
        this.eventBus.emit("show_message", {
          title: __(`Cannot use more credit points than you have `),
          color: "error",
        });
        return;
      }
    ...
    submit(event, payment_received = false, print = false) {
      ...
      if (this.pos_settings_panel.exact_payment) {
        total_paid = this.invoice_doc.payments.reduce((val, obj) => {
          return this.flt(val) + this.flt(obj.amount);
        }, 0);
      ...
      if (
        ...
        this.eventBus.emit("show_message", {
          title: `The Sales Partner is Required, If you don\'t want this message to show please disable the option in the POS Profile`,
          color: "error",
        });
        frappe.utils.play_sound("error");
        return;
          ...
          if (r.message && r.message.length > 0) {
            ...
            if (vm.pos_profile.posa_local_storage) {
              localStorage.setItem(
                ...
                JSON.stringify(vm.sales_persons)
              );
            }
          } else {
            vm.sales_persons = [];
          ...
          if (r.message && r.message.length > 0) {
            ...
            if (vm.pos_profile.posa_local_storage) {
              ...
              localStorage.setItem(
                ...
                JSON.stringify(vm.sales_partners)
              );
            }
          } else {
            vm.sales_partners = [];
