...
export interface Notification extends Required<Pick<NotificationData, 'title' | 'color' | 'timeout' | 'count' | 'key'>> {
  summary: string;
  latestDetail: string;
}
...
export const useToastStore = defineStore("toast", () => {
  ...
  const lastNotificationShownAt = reactive<Record<string, number>>({});
  ...
  function show(data: string | NotificationData) {
    ...
    if (currentNotification.value && currentNotification.value.key === notification.key) {
      mergeNotifications(currentNotification.value, notification);
      updateActiveState();
      return;
    }

    const existingQueued = queue.value.find((item) => item.key === notification.key);
    if (existingQueued) {
  ...
  function normalizeNotification(data: string | NotificationData = {}): Notification {
    // Handle simple string input
    if (typeof data === 'string') {
      return normalizeNotification({ title: data });
    }

    const title = typeof data.title === "string" ? data.title.trim() : (data.message || "");
    const color = data.color || "success";
    const timeout = typeof data.timeout === "number" && data.timeout >= 0 ? data.timeout : DEFAULT_SNACK_TIMEOUT;
    const summary = typeof data.summary === "string" ? data.summary.trim() : "";
    const detail = typeof data.detail === "string" ? data.detail.trim() : (data.text || "");
    const count = Number.isFinite(data.count) && (data.count ?? 0) > 0 ? Math.floor(data.count!) : 1;
  ...
  function shouldThrottleNotification(notification: Notification) {
    return false; 
  }
