 ...
 */
export function withPerf<T extends (...args: any[]) => any>(label: string, fn: T): T {
 ...
 */
export function initLongTaskObserver(label: string = "pos-long-task"): () => void {
 ...
 */
export function logComponentRender(vm: any, componentName: string, phase: string, details: Record<string, any> = {}): void {
    if (!isPerfEnabled() || !vm) {
        return;
    }
