...
export async function load_invoice(context, data = {}, options = {}) {
    ...
    if (context.items.length > 0) {
        context.items.forEach((item) => {
            if (!item.posa_row_id) {
                ...
                item.posa_row_id = context.makeid ? context.makeid(20) : Math.random().toString(36).substr(2, 9);
            }
            if (item.batch_no) {
        ...
        const manualSnapshots = context._snapshotManualValuesFromDocItems ? context._snapshotManualValuesFromDocItems(context.items) : [];
    ...
    if (context.packed_items.length > 0) {
        ...
        context.packed_items.forEach((pi) => {
            if (!pi.posa_row_id) {
                pi.posa_row_id = context.makeid ? context.makeid(20) : Math.random().toString(36).substr(2, 9);
            }
        });
    }
    ...
    context.posting_date = context.formatDateForBackend ? context.formatDateForBackend(data.posting_date || frappe.datetime.nowdate()) : (data.posting_date || new Date().toISOString().slice(0, 10));
