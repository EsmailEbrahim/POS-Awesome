...
export async function _applyServerPricingRules(context, ctx = {}) {
    ...
    updates.forEach((update) => {
        ...
        if (allowServerRateUpdate) {
            ...
            item.discount_amount = context.flt ? context.flt(convertedDiscount, precision) : convertedDiscount;
            item.amount = context.flt ? context.flt(item.rate * item.qty, precision) : item.rate * item.qty;
            item.base_amount = context.flt ? context.flt(baseRate * item.qty, precision) : baseRate * item.qty;
        }
