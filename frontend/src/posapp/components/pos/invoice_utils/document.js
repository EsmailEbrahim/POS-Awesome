...
export function get_invoice_doc(context) {
    ...
    if (context.pos_profile.disable_rounded_total) {
        ...
        doc.base_rounded_total = context.roundAmount ? context.roundAmount(doc.base_grand_total) : Math.round(doc.base_grand_total);
    }
    ...
    doc.posting_date = context.formatDateForBackend ? context.formatDateForBackend(context.posting_date_display) : context.posting_date_display;
    ...
    const companyCurrency = (context.company && context.company.default_currency) || context.pos_profile?.currency || null;
...
export function get_invoice_items(context) {
    ...
    context.items.forEach((item) => {
        ...
        const new_item = {
            ...
            posa_delivery_date: context.formatDateForBackend ? context.formatDateForBackend(item.posa_delivery_date) : item.posa_delivery_date,
        };
