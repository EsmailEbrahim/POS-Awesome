            ...
            items.forEach((item) => {
                ...
                if (updated_item.rate !== undefined || updated_item.price_list_rate !== undefined) {
                    ...
                    if (shouldOverrideRate) {
                        if (force || price) {
                            if (context._applyPriceListRate) context._applyPriceListRate(item, price, priceCurrency);
                        }
...
export function _assignManualOverrideValues(context, item, values = {}) {
    ...
    if (values.amount !== undefined) {
        ...
        item.amount = context.flt ? context.flt(item.qty * item.rate, context.currency_precision) : item.qty * item.rate;
    }
    ...
    if (values.base_amount !== undefined) {
        ...
        item.base_amount = context.flt ? context.flt(item.qty * item.base_rate, context.currency_precision) : item.qty * item.base_rate;
    }
}
