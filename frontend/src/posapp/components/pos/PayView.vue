			...
			<v-col md="8" cols="12" class="pb-2 pr-0">
				...
				>
					...
					<PayInvoicesTable
						...
						@select-row="toggleInvoiceSelection($event, customer_name, (cust) => customersStore.setSelectedCustomer(cust))"
					/>
...
export default {
    ...
    setup() {
        ...
        const load_print_page = (payment_name) => {
            ...
            let url = frappe.urllib.get_base_url() + "/printview?doctype=Payment%20Entry&name=" + payment_name + "&trigger_print=1";
        ...
        const total_outstanding_amount = computed(() => {
            ...
            return outstanding_invoices.value.reduce((acc, cur) => acc + flt(cur?.outstanding_amount || 0), 0);
        });

        const total_unallocated_amount = computed(() => {
            if (!unallocated_payments.value.length) return 0;
            return unallocated_payments.value.reduce((acc, cur) => acc + flt(cur?.unallocated_amount || 0), 0);
        });
        ...
        const filtered_payment_methods = computed(() => {
            ...
            const target = selected_invoices.value[0]?.party_account_currency || selected_invoices.value[0]?.currency || pos_profile.value.currency;
        ...
        const fetchCompanyCurrency = async () => {
            ...
            try {
                const r = await frappe.call({
                    ...
                    args: { doctype: "Company", filters: { name: company.value }, fieldname: "default_currency" }
                });
        ...
        const check_opening_entry = async () => {
            ...
            await checkDbHealth();
            try {
                const r = await frappe.call("posawesome.posawesome.api.shifts.check_opening_shift", { user: frappe.session.user });
                if (r.message) {
        ...
        const syncPendingPayments = async () => {
            ...
            if (pending) {
                proxy?.eventBus?.emit("show_message", { title: `${pending} payment(s) pending for sync`, color: "warning" });
            }
            if (!isOffline()) {
                const result = await syncOfflinePayments();
                if (result?.synced) {
                    proxy?.eventBus?.emit("show_message", { title: `${result.synced} offline payment(s) synced`, color: "success" });
                }
            }
        };
        ...
        watch(selectedCustomer, async (val) => {
