...
export function usePosPaySubmission({
	...
	async function processPayment({ printAfter = false }: { printAfter?: boolean } = {}) {
		...
		try {
			...
			const selectedInvoicesForPayload = selected_invoices.value.map((invoice: any) => ({ ...invoice }));

			const totalSelectedInvoicesAmount = selectedInvoicesForPayload.reduce(
				(acc: number, invoice: any) => acc + flt(invoice?.outstanding_amount || 0),
				0,
			);
			...
			const payload = {
				...
				payment_methods: payment_methods.value.filter((m: any) => flt(m.amount) > 0),
