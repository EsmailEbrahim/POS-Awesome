...
export function useItemsSearch() {
	...
	const updateIndexes = (itemList: Item[], posProfile: POSProfile | null) => {
		...
		const includeSerial = normalizeBooleanSetting(posProfile?.posa_search_serial_no);
		const includeBatch = normalizeBooleanSetting(posProfile?.posa_search_batch_no);
	...
	const performLocalSearch = (term: string, itemList: Item[], itemGroup: string) => {
		if (!term) {
