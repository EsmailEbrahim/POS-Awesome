...
export const useItemsSelectorSettings = ({ getVM, itemSync }) => {
	...
	const toggleItemSettings = () => {
		...
		vm.temp_force_server_items = !!(vm.pos_profile && vm.pos_profile.posa_force_server_items);
	...
	const loadItemSettings = () => {
		...
		if (typeof opts.background_sync_interval === "number") {
			vm.background_sync_interval = normalizeBackgroundSyncInterval(opts.background_sync_interval);
		}
