...
export function useInvoiceItems(invoiceType: Ref<string>) {
    ...
    const saveColumnPreferences = () => {
        try {
            localStorage.setItem("posawesome_selected_columns", JSON.stringify(selected_columns.value));
        } catch (e) {
    ...
    // @ts-ignore
    const formatFloat = (val, prec) => format.methods.formatFloat(val, prec || pos_profile.value?.posa_decimal_precision || 2);

    const shouldEnforceStockLimits = (item: any) => {
        if (pos_profile.value && !parseBooleanSetting(pos_profile.value.posa_validate_stock)) {
            return false;
        }
        if (item.is_stock_item === 0 || item.is_stock_item === false) {
            if (item.is_bundle) {
                const bundleChildren = invoiceStore.packedItems.filter((ch: any) => ch.bundle_id === item.bundle_id);
    ...
    const setFormatedQty = (item: any, field_name: string, precision: number | null, no_negative: boolean, value: any) => {
        ...
        const allowNegativeStock = (parseBooleanSetting(stock_settings.value?.allow_negative_stock) ||
            parseBooleanSetting(item?.allow_negative_stock)) && !blockSaleBeyondAvailableQty.value;

        if (enforceStockLimits && item.max_qty !== undefined && flt(item[field_name], precision) > flt(item.max_qty, precision)) {
            const blockSale = blockSaleBeyondAvailableQty.value || !allowNegativeStock;
            if (blockSale) {
                item[field_name] = item.max_qty;
                parsedValue = item.max_qty;
                toastStore.show({
                    title: __("Maximum available quantity is {0}. Quantity adjusted to match stock.", [formatFloat(item.max_qty, precision)]),
                    color: "error"
                });
            } else {
                toastStore.show({
        ...
        if (typeof calc_stock_qty === "function") calc_stock_qty(item, item[field_name]);
    ...
    const add_one = (item: any) => {
        ...
        if (proposed === 0) {
            removeItem(item, { invoiceStore, items: invoiceStore.items, pos_profile: pos_profile.value });
            return;
        }
    ...
    const update_delivery_charges = (conversionRate: number, precision: number) => {
        if (selected_delivery_charge.value) {
            base_delivery_charges_rate.value = selected_delivery_charge.value.rate;
        } else {
