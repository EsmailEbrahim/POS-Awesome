...
export function useInvoiceCurrency() {
    ...
    const fetch_available_currencies = async () => {
        ...
        try {
            ...
            available_currencies.value = [{ value: defaultCurrency, title: defaultCurrency }];
    ...
    const update_currency_and_rate = async () => {
        ...
        const companyCurrency = (company.value && company.value.default_currency) || pos_profile.value.currency;
        ...
        try {
            ...
            toastStore.show({ title: __("Error updating currency"), color: "error" });
        }
    };

    const update_item_rates = async () => {
        console.log("Updating item rates with exchange rate:", exchange_rate.value);
        ...
        items.forEach((item: any) => {
            ...
            if (selected_currency.value === companyCurrency) {
                ...
                const converted_price = flt(item.base_price_list_rate / conversionRate, precision);
                const converted_rate = flt(item.base_rate / conversionRate, precision);
                const converted_discount = flt(item.base_discount_amount / conversionRate, precision);
    ...
    const roundAmount = (amount: number) => {
        ...
        const baseCurrency = price_list_currency.value || pos_profile.value?.currency;
        if (pos_profile.value?.posa_allow_multi_currency && selected_currency.value !== baseCurrency) {
            return flt(amount, 2);
        }
