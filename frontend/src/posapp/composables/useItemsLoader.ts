...
export function useItemsLoader() {
	...
	async function appendCachedItemsPage() {
		const loadPage = getCtx<LoaderContext["appendCachedItemsPage"]>("appendCachedItemsPage");
	...
	async function loadVisibleItems(reset = false) {
		...
		await initPromise;
		const ensureStorageHealth = getCtx<LoaderContext["ensureStorageHealth"]>("ensureStorageHealth");
		...
		if (Array.isArray(pageItems) && pageItems.length) {
			...
			const itemDetailFetcher = getCtx<ItemDetailFetcher>("itemDetailFetcher");
			...
			loadProgress.value = totalItemCount ? Math.round((items.length / totalItemCount) * 100) : 100;
	...
	async function onVirtualRangeUpdate(_startIndex, _endIndex, _visibleStartIndex, visibleEndIndex) {
		...
		if (!total) {
			const scheduleCardMetricsUpdate = getCtx<LoaderContext["scheduleCardMetricsUpdate"]>(
