...
Release 15.29.2 — May 19, 2026
### 🐞 Bug Fixes
- add POS profile write authorization helper feature 
- add POS profile write authorization helper feature 
- add POS profile write authorization helper feature 
- addItem promises resolve even when invoiceStore.updateItemWithTotals is unavailable 
- address report, item sync, and quantity handling issues 
- guard POS purchase write APIs with profile authorization 
- guard POS purchase write APIs with profile authorization 
- guard POS purchase write APIs with profile authorization 
- memory leak in newaddress.vue file 
- memory leak in newaddress.vue file 
- memory leak in newaddress.vue file 
- removed unsafe v-html in frontend 
- removed unsafe v-html in frontend 
- removed unsafe v-html in frontend 
- route cart item mutations through invoice store totals 
- route cart item mutations through invoice store totals 
- route cart item mutations through invoice store totals 
- v-html XSS in Customer.vue 
- v-html XSS in Customer.vue 
- v-html XSS in Customer.vue 
- validate POS customer write API authorization 
- validate POS customer write API authorization 
- validate POS customer write API authorization 
- pre-import pricing_rules to avoid Py3.14 ModuleLock (import)
- register at /sw.js?v=<build> so deploys swap the SW instance (sw)
_Total: 25 changes_

### ⚡ Performance
- hash entry filenames so deploys cannot serve stale chunks (build)
- pause background item sync while tab is hidden (items)
_Total: 2 changes_

### 📝 Docs
- add multiple refactoring plans 
- add multiple refactoring plans 
- add multiple refactoring plans 
- document intentional backend commit boundaries 
- document intentional backend commit boundaries 
- document intentional backend commit boundaries 
...
- lint TypeScript files with ESLint 
- lint TypeScript files with ESLint 
- bump vuetify 3.7.5 → 3.12.6 (deps)
- split Reports Monolithic into multiple sub components 
_Total: 5 changes_
...
Release 15.29.1 — May 16, 2026
### 🐞 Bug Fixes
- add RTL support & UI improvements for pay sidebar 
- allow split payment overpayments to calculate change 
- cart performance and add incremental addition to qty and totals 
- exchange gain loss issue, use promise.all method in pospay and minor bugs 
- get item call on price list change 
- improve payment UI 
- merge cache now build form stable invoiceStore.itemOrder + itemsData 
- multi-currency payment mode selection and UI improvements 
- multi-currency payment precision and display 
- multi-currency payment precision and display 
- Optimize background item sync detail refresh 
- prevent duplicate browser print prompts 
- prevent negative remaining amount on overpaid sales 
- print in-page when new tab printing is disabled 
- re-evaluate offers and pricing rules after qty edits 
- remove deep item-map watcher 
- skip change payment entry when cash tender can return change 
- Speed up background item sync batching 
- sync active sale totals after offers and pricing rules 
- test pospay mock rtl composable in pay totals sidebar tests 
- triggerUpdateTotals() now touches metadata after recomputing totals, widened the CartItemRow console-log guard regex, removed the quoted console.log("[CartItemRow]..."), dded a safe totals refresh helper. 
- update item amounts and totals during rapid cart merges 
_Total: 22 changes_

### 📝 Docs
- add agent file and docs files for codex 
- update generated API reference [skip ci] 
_Total: 2 changes_

### 🧰 Maintenance
- improve cart item addition performance 
_Total: 1 changes_
